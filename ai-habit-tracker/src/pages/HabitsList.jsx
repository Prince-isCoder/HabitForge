      ...
      <div className="bg-surface border border-border rounded-[2rem] p-6 mb-10 flex flex-wrap items-center gap-6 shadow-sm">
        <div className="flex-1 flex gap-2 flex-wrap">
          {CATEGORIES.map(cat => (
            <button
              key={cat}
              className={`px-5 py-2.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all ${filterCat === cat ? 'bg-primary text-primaryContent shadow-lg shadow-primary/20' : 'bg-background text-textMuted hover:bg-surfaceLight'}`}
  ...
  return (
    <div className={`habit-card ${habit.completed ? "done" : ""}`}>
      <button className={`check-btn ${habit.completed ? "done" : ""}`} onClick={() => onToggle(habit._id)}>
        <Check size={24} className={habit.completed ? "text-primaryContent" : "text-border group-hover:text-primary transition-colors"} />
      </button>
