              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div
                    ...
                    className={`cursor-pointer group relative overflow-hidden rounded-2xl border-2 transition-all p-4 ${theme === 'light' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40 bg-surface'}`}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${theme === 'light' ? 'bg-primary text-white' : 'bg-surfaceLight text-textMuted'}`}>
                        <Sun size={18} />
                      </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="w-1/3 h-12 bg-white rounded-lg border border-slate-200" />
                    </div>
                  </div>

                  <div
                    onClick={() => theme !== 'dark' && toggleTheme()}
                    className={`cursor-pointer group relative overflow-hidden rounded-2xl border-2 transition-all p-4 ${theme === 'dark' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40 bg-surface'}`}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${theme === 'dark' ? 'bg-primary text-white' : 'bg-surfaceLight text-textMuted'}`}>
                        <Moon size={18} />
                      </div>
                      <span className="font-bold text-sm">Dark Mode</span>
                    </div>
                    <div className="flex gap-2">
                      <div className="w-full h-12 bg-[#0B0E14] rounded-lg border border-[#212936]" />
                      <div className="w-1/3 h-12 bg-[#151921] rounded-lg border border-[#212936]" />
                    </div>
                  </div>
                </div>
              </div>
