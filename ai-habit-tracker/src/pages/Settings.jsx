              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className={`flex items-center gap-4 p-5 rounded-2xl border transition-all text-left ${theme === 'light' ? 'bg-primary/5 border-primary shadow-sm shadow-primary/10' : 'bg-surfaceLight/30 border-border/50 hover:border-primary/20'}`}
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${theme === 'light' ? 'bg-primary text-white' : 'bg-surface text-orange-400'}`}>
                      <Sun size={24} />
                    </div>
                    <div>
                      <div className="text-sm font-bold">Light Mode</div>
                      <div className="text-xs text-textMuted">Crisp and clear interface</div>
                    </div>
                  </button>

                  <button
                    onClick={() => setTheme('dark')}
                    className={`flex items-center gap-4 p-5 rounded-2xl border transition-all text-left ${theme === 'dark' ? 'bg-primary/5 border-primary shadow-sm shadow-primary/10' : 'bg-surfaceLight/30 border-border/50 hover:border-primary/20'}`}
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${theme === 'dark' ? 'bg-primary text-white' : 'bg-surface text-primary'}`}>
                      <Moon size={24} />
                    </div>
                    <div>
                      <div className="text-sm font-bold">Dark Mode</div>
                      <div className="text-xs text-textMuted">Gentle on the eyes</div>
                    </div>
                  </button>
                </div>
              </div>
