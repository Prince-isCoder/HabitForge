          ...
          <div className="card !p-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-textMuted uppercase tracking-widest ml-1">Theme Preference</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button
                    onClick={() => setTheme('light')}
                    className={`flex items-center gap-4 p-4 rounded-2xl border transition-all text-left ${theme === 'light' ? 'bg-primary/5 border-primary shadow-sm' : 'bg-surfaceLight/30 border-border/50 hover:border-primary/20'}`}
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-transform ${theme === 'light' ? 'bg-primary text-white scale-110' : 'bg-surface text-orange-400'}`}>
                      <Sun size={20} />
                    </div>
                    <div>
                      <div className="text-sm font-bold">Light Mode</div>
                      <div className="text-[10px] text-textMuted uppercase tracking-tight">Clean & Professional</div>
                    </div>
                  </button>

                  <button
                    onClick={() => setTheme('dark')}
                    className={`flex items-center gap-4 p-4 rounded-2xl border transition-all text-left ${theme === 'dark' ? 'bg-primary/5 border-primary shadow-sm' : 'bg-surfaceLight/30 border-border/50 hover:border-primary/20'}`}
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-transform ${theme === 'dark' ? 'bg-primary text-white scale-110' : 'bg-surface text-primary'}`}>
                      <Moon size={20} />
                    </div>
                    <div>
                      <div className="text-sm font-bold">Dark Mode</div>
                      <div className="text-[10px] text-textMuted uppercase tracking-tight">Modern & Immersive</div>
                    </div>
                  </button>
                </div>
              </div>
