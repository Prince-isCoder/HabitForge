  ...
  return (
    <div style={{ fontFamily: "'Outfit',sans-serif", minHeight: "100vh", background: "var(--background)", color: "var(--text)", padding: "32px", transition: "background-color 0.2s, color 0.2s" }}>
      <style>{`
        ...
        .s-card { background: var(--surface); border: 1px solid var(--border); border-radius: 18px; padding: 28px; margin-bottom: 20px; transition: background-color 0.2s, border-color 0.2s; }
        ...
        .s-input { width: 100%; background: var(--background); border: 1px solid var(--border); border-radius: 12px; padding: 12px 16px; font-size: 14px; color: var(--text); font-family: 'Outfit',sans-serif; outline: none; transition: border-color 0.2s, box-shadow 0.2s; box-sizing: border-box; }
        .s-input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px var(--primary); }
        .s-input:disabled { opacity: 0.4; cursor: not-allowed; }
        .save-btn { background: linear-gradient(135deg,var(--primary),var(--primary-hover)); border: none; border-radius: 12px; color: #fff; padding: 12px 24px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'Outfit',sans-serif; display: flex; align-items: center; gap: 8px; transition: transform 0.15s, box-shadow 0.15s; box-shadow: 0 4px 16px var(--primary); }
        .save-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 24px var(--primary); }
        .logout-btn { background: var(--surface-light); border: 1px solid var(--border); border-radius: 12px; color: var(--danger); padding: 12px 24px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'Outfit',sans-serif; display: flex; align-items: center; gap: 8px; transition: background 0.2s, border-color 0.2s; }
        ...
        .toggle.on { background: linear-gradient(135deg,var(--primary),var(--primary-hover)); }
        ...
        .success-toast { position: fixed; top: 24px; left: 50%; transform: translateX(-50%); background: var(--success); border: 1px solid var(--border); border-radius: 12px; padding: 12px 20px; color: #fff; font-size: 14px; font-weight: 500; z-index: 99; animation: popIn 0.2s ease; }
      ...
      <div className="s-card">
        <div style={{ display: "flex", alignItems: "center", marginBottom: 24 }}>
          <div className="section-icon" style={{ background: "var(--surface-light)" }}><User size={18} color="var(--primary)" /></div>
          <div>
        ...
        <div style={{ display: "flex", align: "center", gap: 32, marginBottom: 24 }}>
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,var(--primary),#06b6d4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700, color: "#fff", flexShrink: 0, boxShadow: "0 0 24px var(--primary)" }}>
      ...
      <div className="s-card">
        <div style={{ display: "flex", alignItems: "center", marginBottom: 20 }}>
          <div className="section-icon" style={{ background: "var(--surface-light)" }}><Bell size={18} color="var(--primary)" /></div>
          <div>
        ...
        <div className="toggle-wrap">
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div className="section-icon" style={{ background: "var(--surface-light)", margin: 0, width: 32, height: 32 }}>
              {theme === 'dark' ? <Moon size={16} color="var(--primary)" /> : <Sun size={16} color="var(--primary)" />}
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500, color: "var(--text)" }}>{theme === 'dark' ? 'Dark' : 'Light'} Mode</div>
              <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Switch between light and dark themes</div>
            </div>
          </div>
      ...
      <div className="s-card">
        <div style={{ display: "flex", alignItems: "center", marginBottom: 20 }}>
          <div className="section-icon" style={{ background: "var(--surface-light)" }}><Shield size={18} color="var(--danger)" /></div>
          <div>
